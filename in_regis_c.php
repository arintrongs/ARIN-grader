...
$classname = $_POST['classname'];
	if($username !="" and $password != "" and $classname != "")
	{
		$strc = mysql_query("SELECT * FROM class");
		...
		while($cResult = mysql_fetch_array($cQuery))
			...
			$cc=1;
		while($classno = mysql_fetch_array($strc))
		{
			$cc++;
		}
		if($c==0)
			{
				$query = "INSERT INTO `members`(`username`,`password`,`class`,`status`) VALUES ('$username','$password','$cc','admin')";
				mysql_query($query) or die("Error [".$query."]");
				$insertc = "INSERT INTO `class`(`name`) VALUES ('$classname')";
				mysql_query($insertc) or die("Error [".$insertc."]");
				
				?>
				<center><h1>Create Class Successful!</h1>
	...
	else
	{
		...
		<h3><a href="register_c.html">Go back</a></h3></center>
		<?php
	}?>
</div>
</div>
